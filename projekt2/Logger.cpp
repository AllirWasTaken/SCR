...
void HandlingThread_SIGUSR1(){
    ...
    while(true){
        ...
        //DUMP
        if(instance->dumpFunction) {
            auto now = instance->GetCurrentDateAndTime();
            std::string fileName = "Dump " + now;
            FILE *f = fopen(fileName.c_str(), "wb");
            if (!f) {
                throw std::runtime_error("Cannot create dump file");
            }
            instance->dumpFunction(f,instance->dumpArgs);
            fclose(f);
        }
...
void Logger::SetMemoryDumpFunction(void (*func)(FILE *, void *),void* args) {
    instance->dumpFunction=func;
    instance->dumpArgs=args;
}
